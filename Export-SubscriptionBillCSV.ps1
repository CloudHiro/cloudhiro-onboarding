Param (
    ...
    [Parameter(Mandatory = $false)]
    [String]$blobContainerName = "billingexports"
)

# Requires -Modules Az.Accounts,Az.Resources,Az.CostManagement

if (-not (Get-InstalledModule -Name "Az.CostManagement")) { Install-Module "Az.CostManagement" -Force }
...
function ExportCostOneTime {
    param ([String]$ExportName, [String]$DefinitionType, [String]$ResourceId, [String]$ContainerName, [String]$CostFolderName, [String]$SubscriptionId, [Int]$NumberOfMonths)
    ...
    New-AzCostManagementExport -Name $ExportName -DefinitionTimeframe Custom -DefinitionType $DefinitionType -DestinationResourceId $ResourceId `
        -DestinationContainer $ContainerName -DestinationRootFolderPath $CostFolderName -Format Csv -Scope $SubscriptionId `
        -TimePeriodFrom $startOfMonth -TimePeriodTo $endOfMonth -ErrorAction SilentlyContinue
}
...
function CreateCostExport {
    ...
    if (-not $(Get-AzCostManagementExport -Name $ExportName -Scope $SubscriptionId -ErrorAction SilentlyContinue)) {
        ...
        if ($NumberOfMonths) {
            $costExport = ExportCostOneTime -ExportName $ExportName -DefinitionType $DefinitionType -ResourceId $ResourceId -ContainerName $ContainerName `
                -CostFolderName $CostFolderName -SubscriptionId $SubscriptionId -NumberOfMonths $NumberOfMonths
        }
        else {
...
foreach ($subscription in $subscriptions) {
    ...
    # # Else create new cost exports
        ...
        if ($subscriptionCheck) {
            ...
            $oneTimeExportParameters = @(
                #                @("$actualCostExportName`_$($subscription.Id)_2_Months", "ActualCost", 2, "one time actual"),
                #                @("$amortizedCostExportName`_$($subscription.Id)_2_Months", "AmortizedCost", 2, "one time amortized"),
                @("$actualCostExportName`_2_Months", "ActualCost", 2, "one time actual", "chiactual"),
                @("$amortizedCostExportName`_2_Months", "AmortizedCost", 2, "one time amortized", "chiamortized"),
                @("$actualCostExportName`_1_Month", "ActualCost", 1, "one time actual", "chiactual"),
                @("$amortizedCostExportName`_1_Month", "AmortizedCost", 1, "one time amortized", "chiamortized")
            )
            $oneTimeExportParameters | ForEach-Object {
                CreateCostExport -ExportName $_[0] -DefinitionType $_[1] -ResourceId $storageAccountResource.ResourceId -ContainerName $blobContainerName `
                    -CostFolderName $blobContainerCostFolderName -SubscriptionId "/subscriptions/$($subscription.Id)" -NumberOfMonths $_[2] -OutputMessage $_[3]
            }
        }
